...
People, teams and projects are imported from Notion and administered here. Each project carries its own record: the budget requested and the budget granted, its start and end, and a high, medium or low priority tier used as a tie-break. Budgets are shown in GPU-hours and as a share of the org pool, and the tables sort by any column. Lane is not a project attribute, since it is chosen per job at submission. This is where the imported structure is checked and kept in step with Notion.

### Planning
This view shows the load on the cluster over time, built from individual jobs the way it comes from the scheduler: the band is the number of GPUs held by running jobs, so it steps up and down as jobs start and finish, with no smoothing between known points. It stacks by team and can be scoped to a team, a project or a person. Narrow windows show the raw job-level steps; wide windows fall back to a daily mean so a year of history stays legible. The proof of concept runs a small scheduler over generated demand: a job starts only when enough GPUs are free, otherwise it queues, so the cluster runs near capacity and jobs wait, and the load bumps the capacity line. This queueing is what the allocation policies exist to shape, and the waits it produces feed the tuning indicators. It carries the reservations that hold GPUs for set windows. Past the current moment the load is forecast from each project's recent rate, capped at the budget it has left, so the forecast tapers as budgets run down. The forecast is shown two ways: capped at the budget, which is the consumption that will actually run and can never exceed capacity, and uncapped, the demand that would follow from the recent rate, which can exceed a budget and shows how far a project threatens to. Reservations, being held GPUs that run around the clock, floor the forecast for their window and so appear as a bump on the load. Aggregated across projects, the uncapped demand gives a cluster-level view of when capacity would be reached. A project on track to exhaust its budget before the period ends is flagged with the date. Operations sets the headroom and the over-subscription factor here, since both are judged against this same live picture. Expected use is therefore never shown above budget, because the budget is a hard cap that stops a project's jobs once it is reached.
...
### Analytics
Longer-run analysis for direction, across periods rather than within one. The indicators are computed per month from the jobs themselves - utilisation, consumed and committed GPU-hours, median wait per lane from each job's submit-to-start delay, the share of jobs that queued over an hour, active projects and projects over budget - tracked past and forecast: utilisation and committed demand against the hardware, median wait by lane, budget spent at mid-period, change requests, projects over budget, unused headroom, and how well use tracks priority. Each can be charted over a configurable period, monthly or quarterly, with actuals solid and the forecast dashed, and policy changes marked on the trend so an indicator's move can be read against the change that preceded it. A scorecard shows every indicator's latest value and its move. This is the evidence for priority and investment decisions, and for tuning the allocation policy itself.
...
### Parameters
Every tunable value the app holds, with how it is used. They are editable only by operations and visible to everyone, set in the policy view except the admission controls, headroom and the over-subscription factor, which are set on the Consumption view. Edits are staged and written to the scheduler only on an explicit apply.
...
- Jobs, one record each from the scheduler's accounting database: submit, start and end time, GPU (TRES) count, owning project and person, and lane. Every load curve is derived from these; the concurrent GPUs at an instant is the sum over jobs running then, so the series is an irregular step function.
